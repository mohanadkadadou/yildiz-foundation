...
interface University {
  ...
  cityEn: string; cityAr: string; cityTr: string; type: string; ranking: number | null;
  descriptionEn: string; descriptionAr: string; descriptionTr: string;
  totalStudents: number | null; isFeatured: boolean; coverImageUrl: string | null;
...
interface UniversitiesClientProps {
  ...
  cities: string[]
  variant?: 'all' | 'government'
  hideTypeFilter?: boolean
}

export function UniversitiesClient({ universities, cities, variant = 'all', hideTypeFilter = false }: UniversitiesClientProps) {
  const { t, locale } = useTranslations()
  const title = variant === 'government' ? t('universities.governmentTitle') : t('universities.all')
  const subtitlePrefix = variant === 'government' ? t('universities.governmentSubtitlePrefix') : t('universities.subtitlePrefix')
  const subtitleSuffix = variant === 'government' ? t('universities.governmentSubtitleSuffix') : t('universities.subtitleSuffix')
    ...
    <div className="pt-24">
        ...
        <div className="container mx-auto text-center">
          ...
          <div className="max-w-2xl mx-auto flex gap-3">
            <div className="flex-1 flex items-center bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl px-4">
              ...
              <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('universities.searchPlaceholder')} className="flex-1 bg-transparent py-4 text-white placeholder:text-white/40 outline-none" />
            </div>
            <button onClick={() => setShowFilters(!showFilters)} className="bg-white/10 border border-white/20 text-white px-5 rounded-2xl flex items-center gap-2 hover:bg-white/20 transition-colors">
              <SlidersHorizontal className="w-4 h-4" /> {t('universities.filters')}
            </button>
          </div>
          {showFilters && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto mt-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
              <select value={city} onChange={e => setCity(e.target.value)} className="bg-white/10 border border-white/20 text-white rounded-xl px-3 py-2 text-sm outline-none">
                <option value="" className="text-gray-900">{t('universities.allCities')}</option>
              ...
              {!hideTypeFilter && (
                <select value={type} onChange={e => setType(e.target.value)} className="bg-white/10 border border-white/20 text-white rounded-xl px-3 py-2 text-sm outline-none">
                  <option value="" className="text-gray-900">{t('universities.allTypes')}</option>
                  <option value="PUBLIC" className="text-gray-900">{t('universities.public')}</option>
                  <option value="PRIVATE" className="text-gray-900">{t('universities.private')}</option>
                </select>
              )}
              <div className={`flex items-center gap-3 ${hideTypeFilter ? 'col-span-3' : 'col-span-2'}`}>
                <span className="text-white/70 text-xs whitespace-nowrap">{t('universities.maxFee')}: ${maxFee.toLocaleString()}</span>
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-6">
          <p className="text-gray-600 dark:text-gray-300">{filtered.length} {t('universities.found')}</p>
          {(q || city || type) && <button onClick={clearFilters} className="flex items-center gap-1.5 text-sm text-red-500 hover:text-red-700"><X className="w-4 h-4" /> {t('universities.clearFilters')}</button>}
        </div>
              ...
              <motion.div key={uni.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="relative h-44 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${uni.type === 'PUBLIC' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>{uni.type === 'PUBLIC' ? `🏛️ ${t('universities.public')}` : `🎓 ${t('universities.private')}`}</span>
                  </div>
                ...
                <div className="p-5">
                  <h3 className="font-bold text-navy-900 dark:text-white mb-1 line-clamp-1">{localize(uni, 'name', locale)}</h3>
                  <div className="flex items-center gap-1 text-gray-500 text-sm mb-3"><MapPin className="w-3.5 h-3.5" />{localize(uni, 'city', locale)}</div>
                  <p className="text-gray-600 dark:text-gray-300 text-sm line-clamp-2 mb-4">{localize(uni, 'description', locale)}</p>
                  <div className="flex items-center justify-between text-sm mb-4">
                    <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400"><BookOpen className="w-4 h-4 text-navy-500" />{uni._count.programs} {t('universities.programs')}</div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>{minFee && <><span className="text-xs text-gray-400">{t('universities.tuition')} </span><span className="font-bold text-navy-900 dark:text-white">${minFee.toLocaleString()}<span className="text-xs font-normal text-gray-400">/yr</span></span></>}</div>
                    <Link href={`/universities/${uni.slug}`} className="flex items-center gap-1.5 bg-navy-800 hover:bg-navy-900 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all">
                      {t('universities.viewDetails')} <ArrowRight className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                </div>
              </motion.div>
