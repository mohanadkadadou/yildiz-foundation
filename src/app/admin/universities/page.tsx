...
export default async function AdminUniversitiesPage() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-6">Universities ({universities.length})</h1>
      <div className="bg-white dark:bg-navy-800 rounded-2xl border border-gray-100 dark:border-navy-700 overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b bg-gray-50 dark:bg-navy-900">
            {['#','Name','City','Type','Programs','Featured','Link'].map(h => <th key={h} className="text-left py-3 px-4 text-gray-500 font-medium">{h}</th>)}
          </tr></thead>
          <tbody>{universities.map(u => (
            <tr key={u.id} className="border-b hover:bg-gray-50 dark:hover:bg-navy-700">
              ...
              <td className="py-3 px-4"><Link href={"/universities/" + u.slug} target="_blank" className="text-blue-500 hover:underline text-xs">View</Link></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
