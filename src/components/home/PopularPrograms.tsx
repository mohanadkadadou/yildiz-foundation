...
interface Program { id: string; slug: string; nameEn: string; nameAr: string; nameTr: string; degreeType: string; language: string; tuitionFeeUSD: number; duration: number; category: string; university: { nameEn: string; nameAr: string; nameTr: string; slug: string } }
    ...
    <section className="py-20 bg-gray-50 dark:bg-navy-950">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <span className="inline-block text-gold-600 font-semibold text-sm uppercase tracking-wider mb-3">{t('programs.eyebrow')}</span>
          <h2 className="text-3xl md:text-4xl font-bold text-navy-900 dark:text-white mb-4">{t('programs.popular')}</h2>
          <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto text-lg">{t('programs.popularSubtitle')}</p>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {programs.map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              ...
              <h3 className="font-bold text-navy-900 dark:text-white text-base mb-1 group-hover:text-navy-700">{localize(p, 'name', locale)}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{localize(p.university, 'name', locale)}</p>
              <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300 mb-4">
                <div className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-navy-500" />{p.duration} {t('programs.years')}</div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href={`/programs/${p.slug}`} className="text-navy-700 dark:text-blue-400 text-sm font-semibold flex items-center gap-1 hover:gap-2 transition-all">
                  {t('programs.details')} <ArrowRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </motion.div>
          ))}
